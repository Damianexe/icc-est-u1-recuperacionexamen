...
public class App {
    public static void main(String[] args) {
        ...
        controller.sortByAgeWithSelection(personas);
        ...
        controller.sortByNameWithInsertion(personas);
        app.printPersonasArray(personas);
    ...
    public void searchAndPrintByAge(PersonaController controller, Persona[] personas, int age) {
        int index = controller.binarySearchbyAge(personas, age);
        if (index != -1) {
            System.out.println("Persona encontrada: " + personas[index].getname() + " (" + personas[index].getAge() + " años) en el índice " + index);
        } else {
    ...
    public void searchAndPrintByName(PersonaController controller, Persona[] personas, String name) {
        int index = controller.binarySearchbyName(personas, name); 
        if (index != -1) {
            System.out.println("Persona encontrada: " + personas[index].getname() + " (" + personas[index].getAge() + " años) en el índice " + index);
        } else {
